                                        ...
                                        <div key={order.id} className="p-5 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                                                ...
                                                {order.coupon_code && order.coupon_discount_yen > 0 && (
                                                    <div>
                                                        <p className="text-gray-500 dark:text-gray-400">クーポン割引</p>
                                                        <p className="font-medium text-emerald-600 dark:text-emerald-400">-{formatPrice(order.coupon_discount_yen)}</p>
                                                    </div>
                                                )}
                                                <div>
                                                    ...
                                                    {order.items.map((item) => (
                                                        <div key={item.id} className="flex justify-between text-sm py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                                                            <div>
                                                                ...
                                                                <p className="text-xs text-gray-500 dark:text-gray-400">SKU: {item.product_sku} × {item.quantity}</p>
                                                            </div>
                                                            <div className="text-right">
                                                                <p className="font-medium text-gray-900 dark:text-white">{formatPrice(item.price_yen)}</p>
                                                                <p className="text-xs text-gray-500 dark:text-gray-400">小計: {formatPrice(item.line_total_yen)}</p>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                            ...
                                            <div key={order.id} className="p-4 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                                <div className="flex justify-between items-center">
                                                    <div>
                                                        ...
                                                        <div className="mt-2">
                                                            <p className="text-sm text-gray-500 dark:text-gray-400">商品: {order.items.slice(0,2).map(item => item.product_name).join(', ')}</p>
                                                            {order.items.length > 2 && <span className="text-sm text-gray-500 dark:text-gray-400">...他{order.items.length - 2}点</span>}
                                                        </div>
                                                        {order.coupon_code && order.coupon_discount_yen > 0 && (
                                                            <div className="mt-1">
