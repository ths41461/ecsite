    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-2xl font-semibold text-[#363842]">安全な支払い</h1>
            <p className="text-sm text-neutral-600">注文番号 #{order_number}</p>
          </div>
          {session_status === 'open' && (
            <span className="bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">確認待ち…</span>
          )}
          {session_status === 'expired' && (
            <span className="bg-rose-100 px-2 py-0.5 text-xs font-medium text-rose-800">セッション期限切れ — 再度開始</span>
          )}
        </div>
        <div className="flex items-center gap-4">
          <a href={start_url || '/checkout'} className="text-sm text-neutral-600">
            チェックアウトに戻る
          </a>
          {cancel_url && allow_cancel && (
            <a href={cancel_url} className="text-sm text-rose-700">
              キャンセルして戻る
            </a>
          )}
        </div>
      ...
      {session_status === 'expired' ? (
        <div className="border p-6 text-center">
          ...
          <a href={restart_url || start_url || '/checkout'} className="inline-block bg-neutral-800 px-4 py-2 text-sm text-white">
            チェックアウトを再度開始
          </a>
        </div>
      ) : (
        <div id="checkout" className="relative min-h-[500px]">
          {!stripeReady && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="flex items-center gap-3 border border-neutral-200 bg-white px-4 py-2 text-sm text-neutral-600">
      ...
      <div className="mt-4 flex flex-col gap-1 text-xs text-neutral-600">
        ...
        <div>
          問題がありますか？{' '}
          {fallback_url ? (
            <a href={fallback_url} className="underline">安全な支払いページを開いてみてください</a>
          ) : (
            <a href={start_url || '/checkout'} className="underline">チェックアウトに戻る</a>
          )}
          。
        </div>
      </div>
