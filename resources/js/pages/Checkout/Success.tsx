  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="mb-2 flex items-center gap-3">
        <h1 className="text-2xl font-semibold text-[#363842]">注文詳細</h1>
        {(() => {
      ...
      <div className="mb-6 py-4">
        <h2 className="mb-3 text-lg font-semibold text-[#363842]">支払い</h2>
        {order.payments?.some((p) => p.processed_at) ? (
          <div className="py-2 text-emerald-700">支払いが処理されました。</div>
        ) : (
          <div className="py-2 text-amber-700">確認を待っています…</div>
        )}
      </div>

      {order.timeline && order.timeline.length > 0 && (
        <div className="mb-6 py-4">
          <h2 className="mb-3 text-lg font-semibold text-[#363842]">注文タイムライン</h2>
      ...
      <div className="flex items-center gap-4">
        <a href="/products" className="text-sm text-neutral-700">買い物を続ける</a>
        <a href="/" className="text-sm text-neutral-700">ホーム</a>
        <a href={`/orders/${encodeURIComponent(order.order_number)}/view`} className="text-sm text-neutral-700" target="_blank" rel="noreferrer">注文を表示</a>
      </div>
    </div>
    </div>
  )
